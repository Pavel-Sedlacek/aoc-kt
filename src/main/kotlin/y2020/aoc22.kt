...
fun main() {
    ...
    println("Day TWENTY-TWO : crab recursive cards")
    println("Part One ${playGame1(makeQueues(input))}")
    println("Part Two ${playGame2(makeQueues(input)).second}")
}
...
fun playGame2(players: Pair<Queue<Int>, Queue<Int>>): Pair<Boolean, Int> {
    ...
    while (true) {
        ...
        if (card1 > player1.size || card2 > player2.size) {
            ...
            val newPlayer1: Queue<Int> = LinkedList()
            player1.forEachIndexed { i, c -> if (i < card1) newPlayer1.offer(c) }
            val newPlayer2: Queue<Int> = LinkedList()

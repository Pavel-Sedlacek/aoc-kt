...
var validTicks = mutableListOf<String>()

fun main() {
    println("Day SIXTEEN : Tickets")
    println("Part One ${partOneSixteen()}")
    println("Part Two ${partTwoSixteen()}")
}
...
fun partOneSixteen(): Int {
    val input = File("src/main/resources/Day16.txt").readText().split("\n\n")
...
fun partTwoSixteen(): Long {
    val (ranges, your, nearby) = File("src/main/resources/Day16.txt").readText().split("\n\n").map{ it.split("\n") }
    ...
    return your[1].split(",").map { it.toInt() } .foldIndexed(1L) { idx, acc, e -> if ((params[idx] and departure) > 0) acc * e else acc }
}